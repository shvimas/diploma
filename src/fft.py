...
class FFT:
    def __init__(self, model: str, args: tuple, alpha=None):
        try:
    ...
    def price(self, pars: tuple):
        ...
        return hf.cut_negative(prices)

    def psi(self, v: ndarray, pars: tuple) -> ndarray:
        # see formula (6) in Carr-Madan
        return exp(-self.r * self.t) * \
               (self.log_return(v - (self.alpha + 1) * 1j, self.r, self.d, self.t, *pars) /
                (self.alpha * self.alpha + self.alpha - v * v + 1j * (2 * self.alpha + 1) * v))
    ...
    def call_price(self, pars: tuple) -> ndarray:
        ...
        try:
            ...
            def integrand(v: ndarray):
                return self.integrand_vectorized(v, pars)

            result = 1 / pi * exp(-self.alpha * log(self.strikes / self.spot)) * \
